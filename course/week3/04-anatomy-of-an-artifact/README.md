...
3. 🧑‍💻 **Logic (read, don't fear):** in the `<script>`, find where it sets the button text and change
   `'Show all'` to `'Show every matter'`. Refresh and click — you just edited behavior.

<details><summary>✅ What to expect</summary>

The tab title changes, the practice-area cell reads "Litigation · Active," the heading recolors, and the
toggle button now says "Show every matter" after you click it. If the button stops working, you likely broke
a quote or removed a `;` in the `<script>` — undo and try a smaller edit.
</details>
